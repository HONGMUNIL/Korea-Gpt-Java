...
/*오버라이딩===
 * 자식 클래스가 부모 클래스에서 상속받은 '메서드'를 자신으 ㅣ상황에 맞게 '재정의'
...
class ChildClass extends ParentClass {
    ...
    //     부모클래스의 메서드를 자식 클래스에서 재정의했는지 판단
    void exercise() {
...
/*1. 오버라이딩 특징
 * 메서드의 시그니처가 일치!
 * 부모클래스에서 정의된 메서드와 동일한 이름, 매개변수 구성을 가져야함.
 *   반환 타입은 같거나 자동 형 변환이 가능하게 작성
 *
 * 2. 오버라이덩 구현 방법
 * -부모클래스의 상속 받은 메서드를 자식 클래스에서 다시 정의
...
public class Overriding {
    public static void main(String[] args) {
        ...
        // 해당상대나 동작을 반드시 가지지만 상속받는 클래스 마다 차이를 두고 싶을 떄
        //===오버로딩 vs 오버 라이딩 비교====
        ...
        // : 메서드명은 동일
        // : 하지만 매개변수의 타입, 개수, 순서 중 적어도 하나 이상은 달라야 함
        // : 반환 타입은 관련이 없다
        // : 메서드의 동작을 다양한 상황에 맞게 조정

        //2)오버라이딩
        // - 상속 관계에 있는 클래스에서 사용
        // : 메서드의 시그니처(반환 타입, 이름, 매개변수)가 같아야 함.
        // : 메서드의 동작을 변경하거나 확장
        // - @Override 어노테이션 사용
        // : 컴파일러에게 전달
