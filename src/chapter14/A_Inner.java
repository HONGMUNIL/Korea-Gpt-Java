...
/*
 * 내부클래스(Inner class) == 중첩 클래스
 *  : 다른 클래스 내부에 선언된 클래스를 의미
 *
 * == 장점 ==
 *  : 코드의 가독성 향상 - 코드의 구조 파악 용이
 *  : 클래스 간의 관계를 명확하게 함 - 내부클래스는 외부클래스의 멤버(필드,메서드)에 접근 가능
 *
 *
 * == 종류 ==
 * 1. (비정적) 내부 클래스 - 가장 기본적인 클래스 형태
 *  : 외부 클래스의 인스턴스에 속함
 *       >> 외부 클래스의 인스턴스 변수 및 메서드에 직접적 접근이 가능
 *
 * 2. 정적(static) 내부 클래스
 *  : 외부 클래스의 정적 멤버에 속함
 ...
 *       >> 해당 메서드 내에서만 사용 가능
 *       >> 메서드의 지역 변수에만 접근 가능
 *        cf)로컬 변수(외부클래스 멤버 변수)가 final인 경우에만 접근 가능
 *
 *
 * 4. 익명(내부) 클래스
 *  : 이름이 없는 내부 클래스
...
class OuterClass {
    ...
    void outerMethod() {
        ...
        String localVar = "메서드 로컨 변수"; //암묵적으로 final 간주(생략)
        ...
        //로컬 클래스의 인스턴스화
        //: 메서드 안에서만 쓰이기 떄문에 인스턴스화로 내부에서 일어남
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        innerClass.didplay(); //외부 클래스 필드에 접근: 외부 클래스의 필드
