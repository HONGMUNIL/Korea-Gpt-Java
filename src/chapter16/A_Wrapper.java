...
*
* cf)기본 데이터타입 : int, short, byte,  long, double, float, boolean, char,  viud
*       >>각 데이터 타입에 해당하는 Wrapper 클래스가 존재
*
* -java.lang 패키지에 정의
* - 사용 용도 예시) 컬렉션 프레임 워크에서는 개체만 저장 가능!
*           >>기본 데이터 형태를 저장하려면 Wrapper클래스로 변환하여 저장
*
* - 참조 데이터 타입
*  Byte, Short, Integer, Long, Float, Boolean, Character, Void, Double
*
*
* +) ===============박싱 & 언박싱=========
*   : 기본타입과 해당 타입의 Wrapper 클래스간의 자동 변환
*   - 자바 컴파일러에 의해 자동으로 처리
*
*
* 1) 박싱
*  : 기본 데이터 타입의 값을 핻아하는 Wrapper 클래스의 객체로 변환하는 과정
*   : int 데이터 타입의 값을 Integer  객체로 변환하는것을 의미
*
*
* 2) 언박싱
*   :Wrapper클래스의 객체를 해당하는 기본 데이터 타입으로 변환하는 과정
*   -Integer 객체의 값을 int 데이터 타입으로 변환하는 것을 의미
*
*  +) 박싱과 언박싱의 장단점
*       장점: 기본 데이터 타입과 객체 타입간의 변환을 자동으로 처리, 코드 작성에 용이
*           : 컬렉션 프레임워크와 같이 객체만 요구하는 API와 호환성이 높음
*
*       단점: 객체를 생성하는 과정에서 추가적인 메모리 사용량이 증가되어 박싱, 언박싱 과정에서 성능 저하
...
//      System.out.pringln() 함수를 사용하여 Wrapper 클래스의 객체 호출 시
//      해당 객체의 toString() 메서드가 자동으로 호출되어 객체가 보유하고 있는 실제 값 이 출력
...
public class A_Wrapper {
    public static void main(String[] args) {
        ...
        //자동 박싱 : 기본 타입의 값을 Integer객체로 변환
        Integer integerObject = i;  //주소를 찾아가서 integer 값의 5를 반환
        ...
        //2) 주요 메서드
        //  - intValue() : Integer 객체를 기본 int 타입을 ㅗ변환
