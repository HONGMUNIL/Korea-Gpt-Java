...
//OOP (Objected Programming)
/* : 객체 지향 프로그래밍
 *
 * =====SOLID 원칙======
 * : 겍체 지향 프로그래밍의 5가지 소프트웨어 개발 원칙
 *  : SRP, OCP, LSP, ISP, DIP
 *  - 소프트웨어 설계에서 유지보수성과 확장성을 높임
 *  - 다양한 디자인 패턴(이 SOLID 설계 원칙을 따라 만들어짐
 *
 *  SOLID 원칙 적용 시 주의점
 *  ! SOLID 원칙은 적용 순서가 없다 X !
 *  ! 5가지 원칙이 모두 필수가 아니다 !
 *  ! 5가지 원칙이 서로 독립된 개념이 아니라 서로 개념적으로 연관 되어 있다
 *
 * cf) 추상화, 다형성, 캡슐화, 상속 (OOP의 4가지 특징)
 *
...
class ReportManager { //보고서 관련업무 담당 클래스
    //보고서 생성
    void generateReport() {
        System.out.println("보고서가 생성 됨");
    }

    //이메일 전송
    void send() {
        System.out.println("이메일이 전송됨"); //이건 SRP설계원칙에 위배됨
    }
}
...
// : 소프트웨어 엔티티(클래스, 모듈, 함수 등)는 확장에는 열려있어야하고, 수정에는 닫혀있어야 한다.
// - 기존 코드를 변경하지 않고 새로운 기능을 추가 할 수 있어야 함.
...
//3. 리스코프 치환 원칙(Liskkov Substitution Principle, LSP)
// : 상위 클래스의 객체를 하위 클래스의 객체로 치환하더라도 프로그램의 동작이 일관되게 유지되어야 함.
// - 자식클래스는 부모클래스의 기능을 완전하게 대체할 수 있음.
...
class Penguin extends Bird {
    @Override
    void fly() { //펭귄은 '새'를 상속 받지만 부모 클래스(Bird)의 fly 메서드가 가지는 동작이 의도대로 구현되지 않는다
        System.out.println("펭귄을 날 수 없습니다.");
    }
} //나쁜 예시
...
class Pororo extends AbstractBird{
    @Override
    public void move(){
        System.out.println("펭귄은 수영할 수 있스빈다");
    }
}
