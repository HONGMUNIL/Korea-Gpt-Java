...
/* ===다형성===
*  : 많은 형태를 가질 수있는 특성
* >>하나의 객체가 여러타입의 인스턴스(객체)로 취급될 수 있는 특성
* >> 상속 + 메서드 오버라이딩 + 클래스 타입 변환
*
* cf) instanceof 연산자
*  : 참조 변수값 instanceof 타입(클래스명)
* >>해당객체가 특정 클래스의 인스턴스인지
...
public class F_Polymorphism {
    public static void main(String[] args) {
        // 다형성 적용
        // 부모ㅓ 클래스 타입의 참조 변수로 자식 클래스 객체를 참조
        Vehicle myVehicle = new Vehicle();
        Vehicle myBus = new Bus();
        Vehicle mySubway = new Subway();
        if(myBus instanceof Vehicle) {
            //myBus 객체가 Vehicl의 인스턴스 이거나( || ) Vehicle 을 상속받은 인스턴스의 경우
            myBus.display();
        }
